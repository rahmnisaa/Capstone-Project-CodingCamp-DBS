...
async def recommend_by_similar_users(request: RecommendationRequest):
    ...
    for similar_user in similar_users:
        user_data = data[data['buyer_id'] == similar_user]
        high_rated_products = user_data[user_data['product_rating'] >= 4.0][['product_id', 'product_name', 'seller_id', 'product_rating', 'product_price']]
        
        for _, row in high_rated_products.iterrows():
            recommended_products.add((row['product_id'], row['product_name'], row['seller_id'], row['product_rating'], row['product_price']))
    
    # Produk yang belum pernah dinilai oleh pengguna target
    target_user_products = data[data['buyer_id'] == user_id]['product_id']
    ...
    for prod in recommended_products:
        product_id, product_name, seller_id, product_rating, product_price = prod
